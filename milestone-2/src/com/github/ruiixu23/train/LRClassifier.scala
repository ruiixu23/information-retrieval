...
/**
 * Created by ruiixu23 on 22/11/14.
 *
 * Implement the Logistic Regression algorithm
 */
class LRClassifier extends Classifier {
  ...
  def train(numOfFeatures: Int, numOfIterations: Int): Unit = {
    // Calculate feature space dimension
    sizeOfVector = tokenIndices.size.min(numOfFeatures) + 1
...
//      for (i <- 0 until sizeOfVector) {
//        theta(i) = random.nextDouble() - random.nextInt(2)
//      }
      thetas += (topic -> theta)
    })
    ...
    for (iteration <- 1 to numOfIterations) {
      ...
      tokenGroups.foreach(group => {
        ...
        vector(sizeOfVector - 1) = 1
      })

      // Update the weight vectors
      topics.foreach(topic => {
        val theta = thetas(topic)
        val score = logistic(theta, vector)
        val update = if (target.contains(topic)) {
          vector * (eta * (1 - topicWeights(topic)) * score)
        } else {
          vector * (eta * topicWeights(topic) * (0 - score))
        }
        thetas += (topic -> (theta + update))
      })
  ...
  private def logistic(theta: SparseVector[Double], vector: SparseVector[Double]): Double = {
    1.0 / (1.0 + math.exp(-1 * (theta dot vector)))
  }

  private def tfIdf(tf: Int, df: Int): Double = {
    tf.toDouble * math.log(numOfDocs.toDouble / df)
  }
}
