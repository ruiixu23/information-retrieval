 ...
 *
 * Utility methods for processing the corpus
 */
object CorpusUtils {
  private val encoding = "UTF-8"

  private val outFileName = "corpus-converted"
  ...
  private val stopWords = Set(
    ...
    "you'd", "you'll", "you're", "you've", "your", "yours", "yourself", "yourselves", "a", "able",
    ...
    "ah", "all", "almost", "alone", "along", "already", "also", "although", "always", "am", "among",
    ...
    "hence", "her", "here", "hereafter", "hereby", "herein", "heres", "hereupon", "hers", "herself",
    ...
    "my", "myself", "n", "na", "name", "namely", "nay", "nd", "near", "nearly", "necessarily",
  ...
  private val processor = new CoreNLPProcessor(withDiscourse = false)

  private def isNumber(token: String): Boolean = token.matches("[+-]?\\d+.?\\d+")

  def tokenizeWithLematizer(text: String): List[String] = {
    val doc = processor.mkDocument(text.toLowerCase)
    processor.tagPartsOfSpeech(doc)
    processor.lemmatize(doc)
    var tokens: List[String] = Nil
    for (sentence <- doc.sentences) {
      sentence.lemmas.foreach(lemmas => tokens = tokens ::: lemmas.toList)
    }
    doc.clear()
    tokens
  }

  def removeStopWords(tokens: List[String]): List[String] = tokens
    .filter(token => token.length() > 2 || isNumber(token))
    .filterNot(token => stopWords.contains(token))

  def convert(path: String, isTestingSet: Boolean): Unit = {
    ...
    val corpusFile = new PrintWriter(new File(directory, outFileName), encoding)

    try {
      println("Converting documents at path " + path);
      var numOfDocsConverted = 0
      ...
      while (corpusReader.hasNext) {
        ...
        if (topics.size > 0 || isTestingSet) {
          val tokens = removeStopWords(tokenizeWithLematizer(doc.getTitle + ". " + doc.getText))
          val tokenGroups = tokens.groupBy(identity)

          corpusFile.println(itemId)
          corpusFile.println(topics.mkString(","))
          corpusFile.println(tokenGroups.size)
          tokenGroups.foreach(group => corpusFile.println(group._2.size + "," + group._1))

          numOfDocsConverted += 1
          if (numOfDocsConverted % 5000 == 0) println(numOfDocsConverted + " documents converted")
        }
      }
    } finally corpusFile.close()
  }
}