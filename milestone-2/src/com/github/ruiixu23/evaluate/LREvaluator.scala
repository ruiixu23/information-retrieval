...
import com.github.ruiixu23.train.LRClassifier
 ...
 *
 * Implement the evaluation functions for the Logistic Regression classifier
 */
class LREvaluator(override val isTestingSet: Boolean) extends Evaluator(isTestingSet) {
  protected val validationSubmitFileName = "classify-ruifeng-xu-l-lr.run"
  protected val testSubmissionFileName = "classify-ruifeng-xu-u-lr.run"

  def evaluate(classifier: LRClassifier,  maxNumOfTopics: Int): Unit = {
    var numOfDocs = 0
  ...
  def submit(classifier: LRClassifier, maxNumOfTopics: Int, path: String): Unit = {
