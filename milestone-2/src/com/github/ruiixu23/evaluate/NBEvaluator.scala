...
import com.github.ruiixu23.train.NBClassifier
 ...
 *
 * Implement the evaluation functions for Naive Bayes classifier
 */
class NBEvaluator(override val isTestingSet: Boolean) extends Evaluator(isTestingSet) {
  protected val validationSubmitFileName = "classify-ruifeng-xu-l-nb.run"
  protected val testSubmissionFileName = "classify-ruifeng-xu-u-nb.run"

  def evaluate(classifier: NBClassifier, maxNumOfTopics: Int): Unit = {
    var numOfDocs = 0
  ...
  def submit(classifier: NBClassifier, maxNumOfTopics: Int, path: String): Unit = {
